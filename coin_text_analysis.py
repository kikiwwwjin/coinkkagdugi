...
# 분석 대상 날짜 리스트 생성 함수(크롤링 기간 설정 : 시작날짜 ~ 현재 or 마지막 날짜)
...
# 뉴스 기사 수집 및 파씽단어 리스트 생성
def article_preprocessing(p_f_list, p_dt):
    # 기사 제목 및 기사 내용 파씽 및 문맥 분석
    all_df = pd.DataFrame()
    for f_nm in tqdm(p_f_list):
        ...
        # 크롤링 파일 불러오기
        try:
            print(file_csv_path+f_nm+'_crawling_'+p_dt+'.csv')
            df = pd.read_csv(file_csv_path+f_nm+'_crawling_'+p_dt+'.csv', encoding='cp949')
            df['ref_site'] = f_nm
    ...
    # 각 문서를 앞서 정의된 단어로 파씽하고 특수문자를 제거 => 파씽된 단어리스트를 생성하여 tokenization에 활용 => 문서간 유사율 확인
    ...
    return all_df, all_passing_words
# article_preprocessing 함수 실행
all_df, all_passing_words = article_preprocessing(p_f_list=f_list, p_dt=today_dt)

# 문서 유사도(정의된 단어 사전 기반)를 통한 비슷한 뉴스 기사 제거(제거 기준 80% 이상)
def document_similarity(p_all_df, p_all_passing_words):
    ...
    tfidf_array = tfidf_fit.transform(p_all_df['doc_content']).toarray()
    ...
    p_all_df.drop(index=[x[0] for x in sml_list], inplace=True)

    return p_all_df
# document_similarity 함수 실행
all_df = document_similarity(p_all_df=all_df, p_all_passing_words=all_passing_words)
...
# 텍스트 분석을 통한 긍부정 스코어 산출 및 워드클라우드(빈도수) 생성
def text_analysis(p_all_df,p_all_word_list,p_anal_dt_list,coin_nm_list,prd_nm_list,prd_jud_list,coin_jud_list,p_dt):
    # 등록시간 순으로 정렬
    ...
    for dt in tqdm(p_anal_dt_list):
        ...
        # 스코어 및 빈도수 데이터 적재
        upload_fnm = file_csv_path + 'score_' + p_dt + '.csv'
        ...
        upload_fnm = file_csv_path + '코인및상품_매핑단어_'+p_dt+'.csv'
        if os.path.isfile(upload_fnm) == False:
        ...
        noun_key_df = pd.DataFrame(data = p_all_word_list, columns=['명사'])
        freq_df = pd.DataFrame({'명사': all_noun})
        freq_df = freq_df[freq_df['명사'].isin(p_all_word_list)].value_counts().reset_index()
        ...
        upload_fnm = file_csv_path + '핵심단어_빈도수_' + p_dt + '.csv'
        if os.path.isfile(upload_fnm) == False:
        ...
        ### 워드클라우드(일자별)
        if int(p_dt) - int(dt.split(' ')[0].replace('-','')) < 7 : # 7일 이내만 워드 클라우드 저장
            freq_dict = dict()
...
# text_analysis 함수 실행
text_analysis(p_all_df=all_df, p_all_word_list=all_word_list, p_anal_dt_list=anal_dt_list
              , coin_nm_list=coin_nm_list, prd_nm_list=prd_nm_list, prd_jud_list=prd_jud_list, coin_jud_list=coin_jud_list
              , p_dt=today_dt)
...
def making_index_result(p_file_csv_path):
    ...
    # 비트코인(바이낸스, 업비트 기준) 시세 및 지표 데이터 적재
    bit_info_df.to_csv(p_file_csv_path + 'bitcoin_price_idx_result_'+dt+'.csv', index=False, encoding='cp949')
    print('각종 지표 산출 및 차트 생성용 데이터 적재 완료')
    print('#'*100)
    return
# 차트용 데이터 생성 및 적재
